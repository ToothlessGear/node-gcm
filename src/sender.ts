...
export default class Sender {
  ...
  send(
    ...
    if (message.options?.data?.from) {
      console.warn(
        "Sending a notification with the 'from' data attribute may invoke a 400 Bad Request by FCM."
      );
    }
    ...
    this.sendNoRetry(
      message,
      recipient,
      (err, response, attemptedRegTokens) => {
        if (err) {
          // Attempt to determine HTTP status code
          const statusCode =
            typeof err === "number" ? err : (err as any).code || 0;

          // 4xx error?
          if (statusCode > 399 && statusCode < 500) {
            debug(
              "Error 4xx -- no use retrying. Something is wrong with the request (probably authentication?)"
            );
            return callback(err);
          }
        ...
        checkForBadTokens(
          response.results,
          attemptedRegTokens,
          function (err, unsentRegTokens, regTokenPositionMap) {
            if (err) {
              return callback(err);
            }
  ...
  sendNoRetry = function (
    message: Message,
    recipient: string | string[],
    callback?: (
      err: unknown,
      response?: Record<string, unknown>,
      attemptedRegTokens?: string[]
    ) => void
  ) {
    if (!callback) {
      callback = function () {};
    ...
    getRequestBody(message, recipient, (err, body) => {
      ...
      const request_options: request.RequiredUriUrl & request.CoreOptions =
        defaultsDeep(
          {
            method: "POST",
            headers: {
      ...
      request(request_options, (err, res, resBodyJSON) => {
        ...
        callback(
          null,
          resBodyJSON,
          (body.registration_ids as string[]) || [body.to as string]
        );
      });
    });
  };
...
function getRequestBody(
  ...
  if (typeof recipientOrCallback == "object") {
    return extractRecipient(
      recipientOrCallback,
      function (err, recipient, param) {
        if (err) {
          return callback(err);
        }
...
function extractRecipient(
  ...
  callback: (
    error: Error | null,
    recipient?: string,
    param?: ReturnType<typeof getParamFromKey>
  ) => void
) {
  ...
  if (recipientKeys.length !== 1) {
    return nextTick(
      callback,
      new Error(
        "Please specify exactly one recipient key (you specified [" +
          recipientKeys +
          "])"
      )
    );
  ...
  if (!value) {
    return nextTick(
      callback,
      new Error("Falsy value for recipient key '" + key + "'.")
    );
  }
  ...
  if (!validator) {
    return nextTick(
      callback,
      new Error("Key '" + key + "' is not a valid recipient key.")
    );
  }
  if (!validator(value)) {
    return nextTick(
      callback,
      new Error(
        "Recipient key '" + key + "' was provided as an incorrect type."
      )
    );
  }
