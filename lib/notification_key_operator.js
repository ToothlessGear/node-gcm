...
function NotificationKeyOperator(key , options) {
    if (!(this instanceof NotificationKeyOperator)) {
        return new NotificationKeyOperator(key, options);
    }
...
NotificationKeyOperator.prototype.performOperationNoRetry = function(op, callback) {
    ...
    if (!NotificationKeyOperation.isValidOp(function(err) {
        if (err) {
    ...
    if (op.registrationIds !== undefined) {
        body[Constants.PARAM_REGISTRATION_IDS] = registrationIds;
    }
