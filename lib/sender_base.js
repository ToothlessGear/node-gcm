...
SenderBase.prototype.sendNoRetry = function(message, recipient, callback) {
    if(!callback) {
        callback = function() {};
    }
    ...
    if (recipient.registrationIds) {
        // Registration IDs take precedence
        body[Constants.JSON_REGISTRATION_IDS] = opts.registrationIds;
    } else if (recipient.notificationKey && recipient.notificationKey.length) {
        body[Constants.PARAM_NOTIFICATION_KEY] = opts.notificationKey; 
    } else {
...
SenderBase.prototype.send = function(message, recipient, retries, callback) {
    ...
    if (recipient && (recipient.notificationKey || 
        ...
        this.sendNoRetry(message, recipient, function lambda(err, result) {
            if (attempt < retries) {
                ...
                if (recipient.registrationIds && recipient.registrationIds.length) {
                    ...
                    if (unsentRegIds.length) {
                        ...
                        setTimeout(function () {
                            self.sendNoRetry(message, recipient, lambda);
                ...
                // If a notification_key is provided (and no registration IDs), retry if an error occurred.
                    ...
                    if (result.success && result.failure && result.failed_registration_ids) {
                        ...
                        // We lose user notification capabilities for those particular devices, but it'll still deliver.

                        recipient.notification_key = null;
                        recipient.registrationIds = failed_registration_ids;
                ...
                // You will probably never arrive at this state, but if you do, we're just going to bail.
                else {
                    callback(err, result);
                }
...
private calculateSleepTime(attempt, backoff) {
