...
Sender.prototype.sendNoRetry = function(message, recipient, callback) {
    ...
    getRequestBody(message, recipient, function(err, body) {
        ...
        request(request_options, function (err, res, resBodyJSON) {
            ...
            if (!res) {
                return callback('response is null');
            }
...
function getRequestBody(message, recipient, callback) {
    ...
    if(typeof recipient == "string") {
        body.to = recipient;
        return process.nextTick(callback.bind(this, null, body));
    }
    ...
    if (typeof recipient == "object") {
        ...
        if (Array.isArray(o.recipient)) {
            body.registration_ids = o.recipient;
            return process.nextTick(callback.bind(this, null, body));
        }
        body.to = o.recipient;
        return process.nextTick(callback.bind(this, null, body));
...
function extractRecipient(recipient) {
    ...
    if(recipientKeys.length !== 1) {
        return { err: new Error("Please specify exactly one recipient key (you specified [" + recipientKeys + "])") };
    }
