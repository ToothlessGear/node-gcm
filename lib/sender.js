...
Sender.prototype.send = function(message, registrationIds, options, callback) {
    ...
    if(typeof options == "object") {
        ...
        backoff = options.backoff || Constants.BACKOFF_INITIAL_DELAY;
    }
    ...
    this.sendNoRetry(message, registrationIds, function(err, result) {
        ...
        if (err) {
            return setTimeout(function() {
                self.send(message, registrationIds, retries - 1, backoff * 2, callback);
            }, backoff);
        }
        ...
        setTimeout(function () {
            ...
            self.send(message, unsentRegIds, retries - 1, backoff * 2, function(err, retriedResult) {
                ...
                result.failure -= unsentRegIds.length - retriedResults.failure;
